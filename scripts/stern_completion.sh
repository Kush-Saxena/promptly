# Generated from stern --completion zsh

# Skip if PROMPTLY_DISABLE_STERN_COMPLETION is set
if [[ -n "$PROMPTLY_DISABLE_STERN_COMPLETION" ]]; then
  return
fi
...
_stern()
{
    ...
    while IFS='\n' read -r comp; do
        ...
        if [ "${comp[1,$endIndex]}" = "$activeHelpMarker" ];then
            __stern_debug "ActiveHelp found: $comp"
            comp="${comp[$startIndex,-1]}"
            if [ -n "$comp" ]; then
                compadd -x "${comp}"
                __stern_debug "ActiveHelp will need delimiter"
                hasActiveHelp=1
            fi
    ...
    if [ $hasActiveHelp -eq 1 ]; then
        if [ ${#completions} -ne 0 ] || [ $((directive & shellCompDirectiveNoFileComp)) -eq 0 ]; then
            __stern_debug "Adding activeHelp delimiter"
            compadd -x "--"
            hasActiveHelp=0
        fi
    fi
    ...
    if [ $((directive & shellCompDirectiveKeepOrder)) -ne 0 ]; then
        __stern_debug "Activating keep order."
        keepOrder="-V"
    fi
